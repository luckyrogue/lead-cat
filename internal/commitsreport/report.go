...
func (b *Builder) section(ctx context.Context, m DevMapping, since, until time.Time, loc *time.Location) (string, error) {
	if m.GitHub == "" {
		return fmt.Sprintf("@%s — GitHub не привязан ⚠️", m.Telegram), nil
	}

	commits, err := b.GH.ListCommits(ctx, m.GitHub, since, until, loc)
	if err != nil {
		return "", err
	}

	if len(commits) == 0 {
		return fmt.Sprintf("@%s (%s) — 0 коммитов ⚠️", m.Telegram, m.GitHub), nil
	}

	var lines []string
	lines = append(lines, fmt.Sprintf("@%s (%s) — %d", m.Telegram, m.GitHub, len(commits)))
	show := commits
