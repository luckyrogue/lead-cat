...
- **Organizer email:** use the existing `s.Store.GetUserByID(ctx, id) (postgres.User, error)` (`auth_repo.go:102`) → `User.Email`. The `meetings` organizer join in `ListScheduleForEmail` is on the `platform_users` table.
...
Build-verified. Read `ListScheduleForEmail` (line ~241) first to match `meetingColsM` and `queryMeetings`. (Organizer email reuses the existing `GetUserByID` — no new method needed.)

- [ ] **Step 1: Add `ListMeetingsOverlapping`**

Append after `ListScheduleForEmail` in `meeting_repo.go`:
...
> **Confirm before building:** `UpdateMeetingInput` has `Date, Start, End *string` (it does — `meeting_update.go`). `Workspace.TZ`, `GetWorkspace`, `GetMeeting`, `GetUserByID` (→ `User.Email`), `ListParticipants`, `SearchEmployeesGlobal` all exist on `*postgres.Store` / `*Services`. `Meeting.OrganizerUserID` is `*uuid.UUID`.
