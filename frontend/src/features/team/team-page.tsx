...
export function TeamPage() {
  ...
  const [drafts, setDrafts] = useState<Record<string, { github: string; gitlab: string }>>({});
  const qc = useQueryClient();

  const { data, isLoading } = useQuery({
    queryKey: ["members", workspaceId],
    queryFn: async () => (await api.get<Member[]>(`/workspaces/${workspaceId}/members`)).data,
    enabled: !!workspaceId,
  });

  const sync = useMutation({
    mutationFn: async () =>
      (await api.post<{ synced: number }>(`/workspaces/${workspaceId}/members/sync-chat`)).data,
    onSuccess: (res) => {
  ...
  const saveVCS = useMutation({
    mutationFn: async (m: Member) => {
      ...
      await api.patch(`/workspaces/${workspaceId}/members/${m.telegram_username}/vcs`, {
  ...
  const draft = (m: Member) =>
    drafts[m.telegram_username] ?? { github: m.github_login, gitlab: m.gitlab_login };

  return (
    <div className="space-y-4">
      <h2 className="text-xl font-bold">Котики команды</h2>
      <p className="text-sm text-muted-foreground">
        Бот должен быть админом в канале. Участники подтягиваются из админов и из тех, кто писал в чат.
      </p>
      ...
      <ul className="space-y-3">
        {data?.map((m) => (
          <li key={m.id} className="space-y-3 rounded-2xl border border-border bg-card p-3">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setDrafts((prev) => ({
                    ...prev,
                    [m.telegram_username]: { ...draft(m), github: e.target.value },
                  }))
                }
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setDrafts((prev) => ({
                    ...prev,
                    [m.telegram_username]: { ...draft(m), gitlab: e.target.value },
                  }))
                }
              />
            </div>
