...
export function LoginPage() {
  ...
  const sendCode = useMutation({
    mutationFn: async () => {
      const path =
        tab === "email" ? "/auth/email/send-code" : "/auth/phone/send-code"
  ...
  const passkeyLogin = useMutation({
    mutationFn: async () => {
      const { data: begin } = await api.post<{
        options: unknown
        session_id: string
      ...
      const { data: finish } = await api.post<{ access_token: string }>(
    ...
    return (
      <div className="mx-auto max-w-md space-y-4 p-6">
        ...
        <p className="text-muted-foreground text-sm">
          Режим разработки — API без входа.
        </p>
        <Button
          className="w-full"
  ...
  return (
    <div className="mx-auto max-w-md space-y-4 p-6">
      ...
      <p className="text-muted-foreground text-sm">
        Email, телефон, passkey или GitHub / GitLab
      </p>
      {config?.dev_otp && (
        <p className="border-border bg-muted/50 text-muted-foreground rounded-lg border px-3 py-2 text-xs">
          Локальный режим: после «Получить код» код появится в уведомлении и
          подставится в поле (письма/SMS нет).
        </p>
      )}
