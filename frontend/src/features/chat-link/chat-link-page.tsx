...
export function ChatLinkPage() {
  ...
  const status = useQuery({
    ...
    queryFn: async () => (await api.get(`/workspaces/${workspaceId}/chat/status`)).data,
    enabled: !!workspaceId,
  });
  const link = useMutation({
    mutationFn: async () =>
      api.post(`/workspaces/${workspaceId}/chat/link`, { chat_id: Number(chatId) }),
    onSuccess: () => {
      status.refetch();
  ...
  const st = status.data as { linked?: boolean; notify_chat_id?: number } | undefined;

  return (
    <div className="space-y-4">
      <h2 className="text-xl font-bold">Привязать логово</h2>
      <p className="text-sm text-muted-foreground">1. Добавь бота в группу 2. /chatid 3. Вставь id сюда</p>
      <p className="text-sm">
        Статус: {status.isLoading ? "…" : st?.linked ? `🐾 ${st.notify_chat_id}` : "не привязан"}
      </p>
