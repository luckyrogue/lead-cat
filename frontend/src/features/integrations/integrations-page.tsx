...
export function IntegrationsPage() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () =>
      (await api.get<Integrations>(`/workspaces/${workspaceId}/integrations`))
        .data,
    enabled: !!workspaceId,
  })
  ...
  const verify = useMutation({
    mutationFn: async () =>
      api.post(`/workspaces/${workspaceId}/integrations/verify`),
    ...
    <div className="space-y-4">
      ...
      {data?.has_vcs_token && (
        <p className="text-muted-foreground text-xs">
          Токен сохранён ({data.vcs_provider})
        </p>
      )}
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          type="password"
          placeholder={
            data?.has_vcs_token ? "новый токен (опционально)" : "token"
          }
          value={token}
      ...
      <div className="space-y-2">
        <Label htmlFor="vcs-org">
          {provider === "gitlab" ? "Group path" : "Org"}
        </Label>
        <Input
          id="vcs-org"
          value={org}
