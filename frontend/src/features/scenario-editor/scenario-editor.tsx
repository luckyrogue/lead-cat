...
function toFlow(def: ScenarioDef) {
  const nodes: Node[] = (def.nodes ?? []).map((n, i) => {
    ...
    return {
      ...
      style: {
        border: `2px solid ${meta?.color ?? "#f97316"}`,
        borderRadius: 12,
        padding: 8,
      },
    }
...
function fromFlow(nodes: Node[], edges: Edge[]) {
  return {
    nodes: nodes.map((n) => ({
      ...
      parameters:
        (n.data as { parameters?: Record<string, unknown> }).parameters ?? {},
    })),
...
export function ScenarioEditor({ workspaceId, scenarioId, onBack }: Props) {
  ...
  return (
    <div className="flex h-[calc(100vh-8rem)] flex-col space-y-3">
      ...
      <div className="border-border bg-card flex-1 overflow-hidden rounded-2xl border">
        <ReactFlow
          nodes={nodes}
          edges={edges}
