...
import { createScenario, fetchScenarios, runScenario } from "@/entities/scenario/api";
...
function ScenariosPage() {
  ...
  if (editId) {
    return (
      <ScenarioEditor workspaceId={workspaceId} scenarioId={editId} onBack={() => setEditId(null)} />
    );
  }
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        ...
        <Button size="sm" className="shrink-0" onClick={() => create.mutate()} disabled={create.isPending}>
      ...
      {data?.map((s) => (
        <div key={s.id} className="rounded-2xl border border-border bg-card p-4">
          ...
          <div className="text-xs text-muted-foreground">{s.enabled ? "включён" : "спит"}</div>
          <div className="mt-2 flex gap-2">
            <Button size="sm" onClick={() => run.mutate(s.id)} disabled={run.isPending}>
              ▶ Прогнать
            </Button>
      ...
      <Button variant="link" className="h-auto p-0" asChild>
        <Link
          to="/scenarios"
          search={{ workspaceId }}
          onClick={() => qc.invalidateQueries({ queryKey: ["scenarios", workspaceId] })}
        >
          Обновить
        </Link>
      </Button>
