...
/** Extra palette-derived surfaces set once on `.tma-frame`. */
export function paletteSurfaceVars(p: CatPalette): CSSProperties {
  return {
    "--tma-hero-gradient": `linear-gradient(135deg, ${hexToRgba(p.accent, p.dark ? 0.28 : 0.16)}, ${hexToRgba(p.accent, p.dark ? 0.1 : 0.04)})`,
    "--tma-hero-paw": hexToRgba(p.accent, p.dark ? 0.12 : 0.1),
    "--tma-auto-banner": `linear-gradient(135deg, oklch(${p.dark ? 0.34 : 0.95} 0.07 25), oklch(${p.dark ? 0.32 : 0.96} 0.05 45))`,
    "--tma-auto-paw": hexToRgba(p.accent, 0.12),
  } as CSSProperties
}
