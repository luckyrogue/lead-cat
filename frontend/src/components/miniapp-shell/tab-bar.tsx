...
export function TabBar() {
  ...
  return (
    <div className="miniapp-tabbar border-miniapp-border bg-miniapp-tg-bar flex items-start border-t">
      {items.map((it) => {
        if (it.key === "_fab") {
          return (
            <div key="_fab" className="relative flex flex-1 justify-center">
              <Link
                to="/meetings/create"
                className="border-miniapp-tg-bar bg-miniapp-accent absolute -top-[30px] flex size-[58px] cursor-pointer items-center justify-center rounded-[20px] border-4 shadow-[0_8px_20px_var(--miniapp-accent-glow)] transition-transform duration-150 active:rotate-90 active:scale-90"
              >
                <CatIcon
                  name="plus"
                  size={28}
        ...
        return (
          <Link
            ...
            className={cn(
              ...
              active ? "text-miniapp-accent" : "text-miniapp-muted"
            )}
          >
            <div
              className={cn(
            ...
            >
              <CatIcon
                name={it.icon}
                size={24}
                className={active ? "text-miniapp-accent" : "text-miniapp-muted"}
                sw={active ? 2.3 : 1.9}
              />
            </div>
            <span
